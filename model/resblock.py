...
class ResBlock_3d(nn.Module):
    def __init__(self, channels, nConvLayers, kSize=3):
        ...
        convs = [Res_Conv_3d(channels) for i in range(nConvLayers)]
        self.convs = nn.Sequential(*convs)
        self.CBAM = CBAM_3d(channels)
...
class Res_Conv_3d(nn.Module):
    def __init__(self, inChannels,  kSize=3):
        super(Res_Conv_3d, self).__init__()
        self.conv = nn.Sequential(*[
            nn.Conv3d(inChannels, inChannels, kSize, padding=(kSize-1)//2, stride=1),
            nn.ReLU()
        ])
